;; doom-miramy-theme.el --- based on doom-miramare, inspired by some atom theme I saw in a Youtube video and obviously gruvbox somehow
(require 'doom-themes)
...
;;
(defgroup doom-miramy-theme nil
  "Options for doom-miramy."
  :group 'doom-themes)

(defcustom doom-miramy-brighter-comments nil
  "If non-nil, comments will be highlighted in more vivid colors."
  :group 'doom-miramy-theme
  :type 'boolean)

(defcustom doom-miramy-padded-modeline doom-themes-padded-modeline
  "If non-nil, adds a 4px padding to the mode-line. Can be an integer to
determine the exact padding."
  :group 'doom-miramy-theme
  :type '(choice integer boolean))

(def-doom-theme doom-miramy
  "A gruvbox variant with comfortable and pleasant colors."

  ;; name        gui       256       16
  ((bg         '("#181a13" "#181a13" nil          )) ; bg1
   (bg-alt     '("#14160e" "#14160e" nil          )) ; bg1
   (bg-alt2    '("#2e2a20" "#2e2a20" "brown"      )) ; bg2 (for region, selection etc.)
   ...
   (magenta    '("#c9999a" "#c9999a" "magenta"))       ; red
   (violet     '("#d3a0bc" "#d3a0bc" "brightmagenta")) ; bright-purple
   (orange     '("#fcb675" "#fcb675" "orange"))        ; bright-orange
   (yellow     '("#e5c672" "#e5c672" "yellow"))        ; bright-yellow
   (teal       '("#c9d779" "#c9d779" "green"))         ; bright-aqua
   (green      '("#c6d68f" "#c6d68f" "green"))         ; bright-green
   ...
   (selection      bg-alt2)
   (builtin        orange)
   (comments       (if doom-miramy-brighter-comments magenta grey))
   (doc-comments   (if doom-miramy-brighter-comments (doom-lighten magenta 0.2) (doom-lighten fg-alt 0.25)))
   (constants      blue)
   (functions      blue)
   (keywords       green)
   (methods        blue)
   ...
   ;; custom categories
   (-modeline-pad
    (when doom-miramy-padded-modeline
      (if (integerp doom-miramy-padded-modeline)
          doom-miramy-padded-modeline
        4)))
...
;;; doom-miramy-theme.el ends here
